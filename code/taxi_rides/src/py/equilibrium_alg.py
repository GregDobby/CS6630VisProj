...
def init_S(day_stat):
    ...
    S = day_stat['match'].copy()
    num_slot = len(total_match)
    num_loc = len(S[0])
    for slot in range(num_slot):
        remain = num_yellow - total_match[slot]
        ...
        for i in range(num_loc):
            ...
            for j in range(num_loc):
                if j in prob[i]:
                    cij = dist[i][j] * 0.1
                    tij = slot + time[i][j]
                    exp_fare += prob[i][j] * (cost[i][j] - cij + V[tij][j])
                    if i == j:
                        tij += gamma
        ...
        for i in range(num_loc):
            ...
            for j in range(num_loc):
                if j in dist[i]:
                    tij = time[i][j] + slot
                    cij = dist[i][j] * 0.1
                    p[j] = np.exp(V[tij][j] - cij)
                    z += p[j]
            # transit
            S[slot][i] += drop[slot][i] - match[slot][i]
            for j in p:
                p[j] /= z
                tij = time[i][j] + slot
                S[tij][j] += round(S[slot][i] * p[j])
    return S
...
def equilibrium(month_stat, day_stat):
    ...
    while diff > threshold:
        ...
        transit(S, V_new, day_stat, month_stat)
        diff = 0
...
for year in years:
    for month in months:
        ...
        for date in dates:
            ...
            equilibrium(month_stat, day_stat)
